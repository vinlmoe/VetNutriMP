...
public class FloatEditingCellAlim extends TableCell<AlimP, Float> {
	  ...
	  DecimalFormat format = new DecimalFormat( "#.0" );
		...
		public void commitEdit(Float item) {
		    ...
		    if (!isEditing() && !item.equals(String.valueOf(getItem()))) {
		        TableView<AlimP> table = getTableView();
		        if (table != null) {
		            TableColumn<AlimP, Float> column = getTableColumn();
		            CellEditEvent<AlimP, Float> event = new CellEditEvent<>(
		                table, new TablePosition<AlimP,Float>(table, getIndex(), column), 
	 ...
	 */
	        textfield.focusedProperty().addListener((ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) -> {
	        	...
	        	if (!newValue & isEditing()) {
	            	System.out.println("END Ficys  OOOO");
	        		commitEdit(Float.parseFloat( floatable(textfield.getText())));
	                setText(floatable(textfield.getText()));
	                return;
	            }
