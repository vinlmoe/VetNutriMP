...
object Mappers {
    ...
    fun AnimalEv.toEntity(includeRelations: Boolean = true): AnimalEntity {
        ...
        if (includeRelations) {
            ...
            this.consultations.forEach { consultation -> consultation.idAnim = this.uuid }
    ...
    fun AnimalEntity.toData(
        ...
        return AnimalEv(
                ...
                specieId = this.specieId ?: "",
                ownerName = this.ownerName ?: "",
                birthdate = this.birthdate?.takeIf { it.isNotEmpty() }?.let { LocalDate.parse(it) },
                race = this.race ?: "",
                summary = this.summary ?: "",
    ...
    fun ConsultationEntity.toData(
        ...
        return ConsultationEv(
                ...
                coefLact = this.coefLact,
                MCS = this.MCS,
                suppVarp = suppVars.map { entity ->
                    SupplementalvariableP(
                            variable = VariableKind.getById(entity.variableKind),
                            varue = entity.value
                    )
                }.toMutableList(),
    ...
    fun AlimentEv.toEntity(includeRelations: Boolean = true): AlimentEntity {
        ...
        if (includeRelations) {
            ...
            this.indicat.forEach { indication ->
                IndicationAlimentEntity(refAliment = this.uuid, indication = indication.coef)
            }
        }
    ...
    fun AlimentEntity.toData(
        ...
        return AlimentEv(
                ...
                indicat = indications.mapNotNull { AlimIndic.byCoef(it.indication) }.toMutableList()
        )
    }
