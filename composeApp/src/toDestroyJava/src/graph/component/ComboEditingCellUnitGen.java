...
public class ComboEditingCellUnitGen extends TableCell<NutrientRefP, UnitP> {
	
    private ComboBox<String> comboBox;

    public ComboEditingCellUnitGen() {
    ...
    private void createComboBox() {
    	 ObservableList<String> targetData     = FXCollections.observableArrayList();
    			 for (UnitEnum t :UnitEnum.values()) {
    				 if ( t.getIDFamily()== ((NutrientRefP) this.getTableRow().getItem()).getUnit().getUnit().getIDFamily() )  {
...
comboBox.setValue(getTarget().getNomS());
    	comboBox.setOnKeyPressed(e -> {
            if (e.getCode().equals(KeyCode.ESCAPE)) {
                commitEdit(new UnitP(UnitEnum.getByName(comboBox.getSelectionModel().getSelectedItem(),  ((NutrientRefP) this.getTableRow().getItem()).getUnit().getUnit().getIDFamily())));
                setText(getTarget().getNomS());
                setGraphic(null);
            }
        });
