...
public class FloatEditingCellWeight extends TableCell<WeightDateP, Float> {
	...
	public void commitEdit(Float item) {
	    ...
	    if (!isEditing() && !item.equals(String.valueOf(getItem()))) {
	        TableView<WeightDateP> table = getTableView();
	        if (table != null) {
	            TableColumn<WeightDateP, Float> column = getTableColumn();
	            CellEditEvent<WeightDateP, Float> event = new CellEditEvent<>(
	                table, new TablePosition<WeightDateP,Float>(table, getIndex(), column), 
