...
/** Implémentation Desktop du FileService */
actual class FileService {
    actual suspend fun getBackupDirectory(): PlatformFile {
        return withContext(AppDispatchers.IO) {
            val userHome = System.getProperty("user.home")
            val backupDir = PlatformFile.create("$userHome/.vetnutri_mp/backups")
            backupDir.mkdirs()
            backupDir
        }
    }
    ...
    actual suspend fun getDataDirectory(): PlatformFile {
        return withContext(AppDispatchers.IO) {
            val userHome = System.getProperty("user.home")
            val dataDir = PlatformFile.create("$userHome/.vetnutri_mp/data")
            dataDir.mkdirs()
            dataDir
        }
    }
